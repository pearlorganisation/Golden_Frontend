        ...
        {Array.isArray(subject) &&
          subject?.map((item) => (
            ...
            >
              ...
              {item.banner?.[0]?.secure_url && (
                <div
                  className="relative w-full h-48 sm:h-64 md:h-72 lg:h-80 xl:h-96 overflow-hidden rounded-lg bg-gray-100"
                  aria-label={item.name}
                >
                  <div
                    className="w-full h-full bg-cover bg-center bg-blue-700"
                    style={{
                      backgroundImage: `url(${item.banner[0].secure_url})`,
                    }}
                  ></div>
